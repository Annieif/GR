...
> 利用 **GitHub Actions** 每日自动跑一次中文 BERT 的轻量微调。
...
| 数据 | `data/corpus.txt`(仓库自带) | 古诗、现代文、谚语、技术文本混排,~200 行 UTF-8 |
| Runner | `ubuntu-latest`(免费,2 核 7G) | 够用,加缓存后单次训练 < 10 分钟 |

如果想要更小的模型(几十 MB),可换:

- `uer/chinese_roberta_L-4_H-256`(~24MB,效果较弱)
- `hfl/chinese-electra-180g-small-discriminator`(~100MB)
...
- 也可在 Actions 页面手动触发 `Daily Chinese BERT Fine-tune`

### 2. 手动触发并自定义参数
在 Actions 页面 `Run workflow` 时可以传:
...
### 4. 替换或扩充训练数据
直接编辑 `data/corpus.txt`,每行一句 UTF-8 中文。
越大越多样,微调效果越明显(目前 ~200 行是 demo 规模)。

## 🔍 查看结果

每次 workflow 跑完会产生两个 artifact:

- `finetuned-model-<run_id>` —— 完整的 `output/` 目录(模型 + tokenizer + 训练日志)
- `training-logs-<run_id>` —— 累积的 `logs/history/` 训练日志
...
- **手动触发**: 支持 `workflow_dispatch` 调试
- **日志持久化**: 训练日志通过 `git commit` 推回 main,留痕每一天的训练曲线

## 📝 实验性

- 训练语料只有 ~200 行,效果有限 —— 这是**有意的**,目的是看「每日小步微调 + 累积」能不能跑通
- 想要真正改善下游任务,请扩充 `data/corpus.txt` 或换成 LoRA / SFT 任务
